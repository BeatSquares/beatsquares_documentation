...
- Manual editor styles now support responsive designs, ensuring your content looks great on any device.
...
- **Unified ContentImage model and backend-owned template context**: Enjoy a consistent image experience across your content.
- **Anchor Curation Agent for poll-anchored segments from related articles**: Enhance your polls with relevant article segments.
- **Inject Brevo campaign ID into newsletter outbound links for internal click tracking**: Track clicks in your newsletters with embedded campaign IDs.
...
- **Introduce MediaItemType registry extension point (prep for BEA-2637)**: Get ready for future enhancements with a new registry extension.
- **E2E test coverage for sub-square picker and blueprint validation**: Enjoy more reliable features with improved testing.

**Bug Fixes & Stability**

- Prevent duplicate content issues in RSS feed article updates.
- Avoid creating empty segments when adding new sub-media items.
- Fix the issue where the Publish button gets stuck in newsletters.
...
- Pre-commit checks in Docker now only format and lint changed lines.
...
- [HOTFIX] Ensure RSS feeds keep HTML attributes and improve editing strategies for linked segments.
...
- **User Management tab — admin self-serve user management**: Manage users independently with a new admin tab.
- **Open Org Settings tabs (Sources / CDN / Brevo) to Admins**: Admins can now access and manage organization settings.
- **Send a bilingual welcome email on user creation via AWS**: New users receive a welcome email in two languages.
- **Add single/multiple choice options for WhatsApp polls**: Create engaging polls with single or multiple choice options.
...
- **Migrate RSS spiders from Scrapy to the RSS_FEED flow**: Experience improved RSS feed processing.
- **Exclude specific RSS spiders from the migration process**: Keep certain spiders unchanged for better control.
- **Introduce CONTENT_TYPE_REGISTRY for content-type dispatch**: Manage content types more effectively.
- **Add content_type routing to the RSS_FEED pipeline**: Enhance content routing in RSS feeds.
- **Redesign bilingual welcome email HTML and copy**: Enjoy a fresh look and feel for welcome emails.
- **Add pagination and search to the organization user list**: Find users faster with new search and pagination features.
- **Migrate the add-new-square page to a dialog in the new**: Experience a smoother interface for adding new squares.
- **Add display-name support in the email "From" header (`from_name`)**: Customize the sender name in emails.

**Bug Fixes & Stability**

- Fix errors when reading and handling data in Firecrawl.
...
- [HOTFIX] Fix issues with MongoDB data writing and date-time handling.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: Ensure consistent podcast information with a unified data structure.
- **Podcast segments — structured dialogue UI and backend sync**: Enjoy a seamless podcast editing experience with synchronized data.
...
- **Generate audio preview for highlighted text segment**: Create audio previews for selected text to enhance your content.
