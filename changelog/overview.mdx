...
- **Add nullable hanko_user_id to users with unique constraint**  
  Users can now have a unique ID that is optional, making it easier to manage user accounts.

- **Hanko JWT dual-auth in require_login and user linkage**  
  Enjoy enhanced security with dual authentication for logging in and linking user accounts.

- **Integrate Hanko auth — frontend elements, JWT validation, and user linkage**  
  Experience seamless login and account linking with improved authentication features.

- **Hanko webhook, JWT validation, and user identity sync on backend**  
  Your user identity is now more secure and synchronized across our systems.

- **Add Hanko Admin API client and bulk user migration tooling**  
  Administrators can now easily manage and migrate multiple user accounts.

- **Add Hanko user id, dual auth, and cutover email templates**  
  Receive clear and helpful email notifications during account changes.

- **Explicit segment blueprint linkage between sub-square and main square**  
  Enjoy better organization with clearer connections between different sections.

- **Add content_image_urls to DataSourceModel for static event images**  
  Easily manage and display event images with the new image URL feature.

- **Sync structured_data from selected option to segment and pass title/structured_data to strategy builder**  
  Benefit from improved data consistency and strategy planning.

**Improvements**

- **Add Sentry ↔ LangSmith bidirectional trace correlation**  
  Enjoy more reliable performance monitoring with enhanced trace correlation.

**Bug Fixes & Stability**

- Scope audio preview player to the variant it was generated for  
  Audio previews now play correctly for their intended versions.

**Other Changes**

- Updated GitHub Actions versions  
  Enjoy smoother operations with the latest updates to our automation tools.

- Infrastructure: Dockerfile scripts directory, postgres password encoding fixes  
  Benefit from improved system security and performance.
...
- **Migrate podcast dialog to structured_data as the single source of truth**  
  Podcast information is now more consistent and reliable.

- **Podcast segments — structured dialogue UI and backend sync**  
  Enjoy a more organized and synchronized podcast experience.

- **Add webhook notification delivery for published podcast audio**  
  Get instant notifications when new podcast episodes are available.

- **Add dynamic poll option add/remove controls in WhatsApp editor**  
  Easily manage poll options in your WhatsApp messages.

- **Generate audio preview for highlighted text segment**  
  Quickly listen to audio previews of highlighted text for better content review.
