...
- **Introduce ContentDocument hierarchy with Beanie is_root polymorphism**: Organize content more effectively with a new structure.
- **Add unified items_in_segment field and backward-compat validator**: Simplify content management with a new field that maintains compatibility.
- **Switch all consumers to ContentDocument and items_in_segment**: Streamline processes by using a unified content format.
...
- **Civey API poll importer with Basic Auth, cron dispatch, and Celery tasks**: Import polls securely and automate scheduling.
- **Add Kraushaar Ferienwohnungen data source spiders**: Access new data sources for more content variety.
...
- **Add Playwright E2E test harness with Claude context and skills**: Enhance testing with a comprehensive test framework.
- **Kraushaar apartment import — sitemap + JSON-LD extraction**: Import apartment data efficiently using sitemaps and JSON-LD.
...
- **Unify source_url → url on ContentDocument**: Simplify content management with a consistent URL field.
- **Move @traceable decorator to execute_strategy_for_variants for LangSmith tracing**: Enhance tracing capabilities for better tracking.

**Bug Fixes & Stability**

- Prevent squares segment skeleton flash during navigation: Enjoy smoother navigation without visual glitches.
- Fix e2e tests after content model refactor: Ensure tests run smoothly after updates.
- Fix all skipped tests from content model refactor (BEA-2594 through BEA-2601): Resolve issues with previously skipped tests.
...
- **Add nullable hanko_user_id to users with unique constraint**: Enhance user management with a new, flexible ID system.
- **Hanko JWT dual-auth in require_login and user linkage**: Strengthen security with dual authentication.
- **Integrate Hanko auth — frontend elements, JWT validation, and user linkage**: Seamlessly connect users with enhanced authentication features.
- **Hanko webhook, JWT validation, and user identity sync on backend**: Ensure user data is consistently updated and secure.
- **Add Hanko Admin API client and bulk user migration tooling**: Manage users efficiently with new admin tools.
- **Add Hanko user id, dual auth, and cutover email templates**: Streamline communication with new email templates.
...
- **Add Sentry ↔ LangSmith bidirectional trace correlation**: Enhance error tracking with improved trace correlation.
...
- Updated GitHub Actions versions: Keep CI/CD processes up-to-date.
- Infrastructure: Dockerfile scripts directory, postgres password encoding fixes: Improve infrastructure reliability and security.
