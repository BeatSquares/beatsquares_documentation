...
- [HOTFIX] We fixed an issue to ensure that certain web content remains editable and displays correctly.
...
- **Add single/multiple choice options for WhatsApp polls**: Create polls with single or multiple choice questions on WhatsApp.
- **Trigger WhatsApp article suggestions on article**: Get article suggestions directly in WhatsApp.
- **Add URL path filters and enrichment concurrency to RSS**: Enjoy improved filtering and processing of RSS feeds.

**Improvements**

- **Migrate RSS spiders from Scrapy to the RSS_FEED flow**: RSS feeds are now processed more efficiently.
- **Exclude specific RSS spiders from the migration process**: Certain RSS feeds remain unchanged to ensure stability.
- **Introduce CONTENT_TYPE_REGISTRY for content-type dispatch**: Content is organized better for easier access.
- **Add content_type routing to the RSS_FEED pipeline**: RSS feeds are sorted more effectively.
- **Redesign bilingual welcome email HTML and copy**: The welcome email looks better and is easier to read.
- **Add pagination and search to the organization user list**: Find users faster with new search and pagination features.
- **Migrate the add-new-square page to a dialog in the new**: Adding new content is now simpler with a dialog box.
- **- Add display-name support in the email "From" header (`from_name`)**: Emails now show a friendly sender name.

**Bug Fixes & Stability**

- Fix read errors in Firecrawl scraping and handling of: We resolved issues with data collection and processing.
- Fix broken local LocalStack by pinning to the community: Local development tools now work reliably.
...
- [HOTFIX] , BEA-2681, BEA-2547: We fixed issues with data storage and date handling to improve reliability.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: Podcast information is now more accurate and consistent.
- **Podcast segments — structured dialogue UI and backend sync**: Podcast segments are easier to manage and update.
- **Add webhook notification delivery for published podcast audio**: Get instant notifications when new podcast audio is published.
- **Add dynamic poll option add/remove controls in WhatsApp editor**: Easily customize polls in WhatsApp.
- **Generate audio preview for highlighted text segment**: Listen to a preview of selected text before publishing.
