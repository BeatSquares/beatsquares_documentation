...
- [HOTFIX] We fixed an issue to ensure that RSS feeds display correctly and can be edited without problems.
...
- **User Management tab — admin self-serve user management**: Admins can now manage users directly from a new tab.
- **Open Org Settings tabs (Sources / CDN / Brevo) to Admins**: Admins have access to more settings for better control.
- **Send a bilingual welcome email on user creation via AWS**: New users receive welcome emails in two languages.
- **Add single/multiple choice options for WhatsApp polls**: Create polls with single or multiple choice answers on WhatsApp.
- **Trigger WhatsApp article suggestions on article**: Get article suggestions directly in WhatsApp.
- **Add URL path filters and enrichment concurrency to RSS**: Enjoy more precise filtering and faster updates in RSS feeds.

**Improvements**

- **Migrate RSS spiders from Scrapy to the RSS_FEED flow**: We improved RSS feed processing for better performance.
- **Exclude specific RSS spiders from the migration process**: Certain RSS feeds remain unchanged for stability.
- **Introduce CONTENT_TYPE_REGISTRY for content-type dispatch**: We organized content types for more efficient handling.
- **Add content_type routing to the RSS_FEED pipeline**: RSS feeds now route content types more effectively.
- **Redesign bilingual welcome email HTML and copy**: Welcome emails look better and are easier to read.
- **Add pagination and search to the organization user list**: Easily find users with new search and pagination features.
- **Migrate the add-new-square page to a dialog in the new**: Adding new items is now more streamlined with a dialog box.
- **- Add display-name support in the email "From" header (`from_name`)**: Emails now show a display name for better recognition.

**Bug Fixes & Stability**

- Fix read errors in Firecrawl scraping and handling of: We resolved issues with data reading and processing.
- Fix broken local LocalStack by pinning to the community: We stabilized LocalStack for smoother operations.

**Other Changes**

- Rename `Role.STAFF` to `Role.SUPER_ADMIN` across the: We updated role names for clarity and consistency.
...
- [HOTFIX] , BEA-2681, BEA-2547: We fixed issues with data storage and date handling to prevent errors.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: Podcast data is now more reliable and consistent.
- **Podcast segments — structured dialogue UI and backend sync**: Podcast segments sync smoothly between the interface and backend.
- **Add webhook notification delivery for published podcast audio**: Get instant notifications when podcast audio is published.
- **Add dynamic poll option add/remove controls in WhatsApp editor**: Easily manage poll options in WhatsApp.
- **Generate audio preview for highlighted text segment**: Listen to a preview of selected text segments in audio form.
