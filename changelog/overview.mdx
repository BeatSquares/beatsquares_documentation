...
- [HOTFIX] We fixed an issue to ensure that specific parts of RSS feeds remain editable and display correctly.
...
- **Add single/multiple choice options for WhatsApp polls**: Create polls with single or multiple choice options on WhatsApp.
...
- **Migrate RSS spiders from Scrapy to the RSS_FEED flow**: We streamlined RSS feed processing for better performance.
- **Exclude specific RSS spiders from the migration process**: Certain RSS feeds remain unchanged for stability.
- **Introduce CONTENT_TYPE_REGISTRY for content-type dispatch**: We improved how different content types are managed.
- **Add content_type routing to the RSS_FEED pipeline**: Content types are now better organized in RSS feeds.
...
- **Migrate the add-new-square page to a dialog in the new**: Adding new items is now more intuitive with a dialog box.
- **- Add display-name support in the email "From" header (`from_name`)**: Emails now show a display name for better recognition.

**Bug Fixes & Stability**

- Fix read errors in Firecrawl scraping and handling of: We resolved issues with data scraping to improve reliability.
- Fix broken local LocalStack by pinning to the community: LocalStack now works correctly with the latest updates.
...
- [HOTFIX] , BEA-2681, BEA-2547: We fixed errors related to data storage and date handling.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: Podcast information is now more organized and reliable.
...
- **Generate audio preview for highlighted text segment**: Listen to a preview of selected text in audio format.
