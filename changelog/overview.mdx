...
- We fixed issues that caused errors when saving data and problems with date and time formats, ensuring smoother performance.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: We streamlined podcast management, making it easier to keep everything organized and up-to-date.
- **Podcast segments — structured dialogue UI and backend sync**: Enjoy a more intuitive interface for podcast segments that stays in sync with the backend.
- **Add webhook notification delivery for published podcast audio**: Get instant notifications when your podcast audio goes live.
- **Add dynamic poll option add/remove controls in WhatsApp editor**: Easily manage poll options in WhatsApp with simple add and remove controls.
- **Generate audio preview for highlighted text segment**: Quickly listen to a preview of any selected text to ensure it sounds just right.
