...
- The manual editor now correctly applies responsive styles, ensuring your content looks great on all devices.
...
- **LinkedIn output via Zernio — backend foundation**: Prepare content for LinkedIn sharing with the new backend setup.
- **Unified ContentImage model and backend-owned template context**: Enjoy a consistent image experience with the new unified model.
- **Anchor Curation Agent for poll-anchored segments from related articles**: Automatically generate poll-based content segments from related articles.
...
- **Content import via URL — framework + Article**: Import content directly from a URL with ease.
- **Parent media-item link in newsletter sub-media-item editor**: Easily access parent media items while editing newsletter content.
- **Content import — Poll (manual add + Civey URL import)**: Import polls manually or via Civey URLs for enhanced flexibility.

**Improvements**

- **Introduce MediaItemType registry extension point (prep for BEA-2637)**: Prepare for future enhancements with the new media item registry.
...
- Ensure newsletter segments remain intact when reverting changes.
- Maintain visual elements like dividers in newsletters.

**Other Changes**

- Move terraform code to its new home at BeatSquares/terraform.
- Docker now only checks and formats changed lines, saving time.
...
- **Add single/multiple choice options for WhatsApp polls**: Create engaging polls with multiple choice options on WhatsApp.
- **Trigger WhatsApp article suggestions on article**: Get article suggestions directly on WhatsApp for easy sharing.
- **Add URL path filters and enrichment concurrency to RSS**: Filter and enhance RSS feeds more efficiently.

**Improvements**

- **Migrate RSS spiders from Scrapy to the RSS_FEED flow**: Experience improved RSS feed handling with the new system.
- **Exclude specific RSS spiders from the migration process**: Maintain control by excluding certain spiders from updates.
- **Introduce CONTENT_TYPE_REGISTRY for content-type dispatch**: Manage content types more effectively with the new registry.
- **Add content_type routing to the RSS_FEED pipeline**: Route content types seamlessly within the RSS feed system.
- **Redesign bilingual welcome email HTML and copy**: Enjoy a fresh look and feel for bilingual welcome emails.
...
- Fix issues with reading errors during Firecrawl scraping.
- Resolve problems with LocalStack by using a stable version.
...
- [HOTFIX] Fix issues with MongoDB data writing and date handling to ensure smooth operation.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: Ensure consistent podcast data with a unified structure.
