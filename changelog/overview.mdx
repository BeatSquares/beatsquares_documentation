...
- [HOTFIX] We fixed an issue that ensures certain parts of your RSS feeds remain editable and display correctly.
...
- **Add single/multiple choice options for WhatsApp polls**: Create more engaging polls with new answer options.
...
- **Migrate RSS spiders from Scrapy to the RSS_FEED flow**: Experience smoother RSS feed updates.
- **Exclude specific RSS spiders from the migration process**: Certain feeds remain unchanged for stability.
- **Introduce CONTENT_TYPE_REGISTRY for content-type dispatch**: Content is now better organized and managed.
- **Add content_type routing to the RSS_FEED pipeline**: RSS feeds are processed more efficiently.
...
- **Migrate the add-new-square page to a dialog in the new**: Adding new items is now more intuitive.
- **- Add display-name support in the email "From" header (`from_name`)**: Emails now show a friendly sender name.

**Bug Fixes & Stability**

- Fix read errors in Firecrawl scraping and handling of: We improved data scraping reliability.
- Fix broken local LocalStack by pinning to the community: LocalStack works correctly without interruptions.

**Other Changes**

- Rename `Role.STAFF` to `Role.SUPER_ADMIN` across the: We updated role names for clarity.
...
- [HOTFIX] , BEA-2681, BEA-2547: We resolved issues with database errors and date handling.
...
- **Generate audio preview for highlighted text segment**: Listen to a preview of selected text segments.
