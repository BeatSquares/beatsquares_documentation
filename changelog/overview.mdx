...
- We fixed issues that caused problems when saving data and handling dates, ensuring smoother performance.
...
- **Migrate podcast dialog to structured_data as the single source of truth**: We improved how podcast information is stored, making it more reliable and consistent.
- **Podcast segments — structured dialogue UI and backend sync**: Enjoy a more organized podcast editing experience with better synchronization between the interface and the backend.
- **Add webhook notification delivery for published podcast audio**: Get instant notifications when your podcast episodes are published.
- **Add dynamic poll option add/remove controls in WhatsApp editor**: Easily add or remove poll options while editing messages in WhatsApp.
- **Generate audio preview for highlighted text segment**: Listen to a quick audio preview of any selected text to ensure it sounds just right.
